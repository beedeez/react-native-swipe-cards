...
export default class SwipeCards extends Component {
	static propTypes = {
		...
		hasTopAction: PropTypes.bool,
		hasBottomAction: PropTypes.bool,
		hasLeftAction: PropTypes.bool,
		...
		renderBottom: PropTypes.oneOfType([PropTypes.func, PropTypes.bool]),
		...
		this._panResponder = PanResponder.create({
			...
			onPanResponderRelease: (e, { vx, vy, dx, dy }) => {
				...
				if (
					...
					(hasSwipedVertically && this.props.hasTopAction)
				) {
					let cancelled = false;
					...
					const hasMovedDown =
						hasSwipedVertically && this.state.pan.y._value > 0;
					...
					if (this.props.smoothTransition) {
						...
						this.cardAnimation = Animated.decay(this.state.pan, {
							velocity: {
								x: animationDirection === 'horizontal' ? velocity : vy,
								y: animationDirection === 'vertical' ? velocity : vy
							},
							deceleration: 0.98
						});
	...
	renderTop() {
		...
		let topOpacity = pan.y.interpolate({
			inputRange: [-SWIPE_THRESHOLD, -(SWIPE_THRESHOLD / 2)],
			outputRange: [1, 0],
			extrapolate: 'clamp'
		});
		let topScale = pan.x.interpolate({
			inputRange: [-SWIPE_THRESHOLD, 0, SWIPE_THRESHOLD],
			outputRange: [0, 1, 0],
			extrapolate: 'clamp'
		});
		let animatedTopStyles = {
		...
		if (this.props.showTop) {
			return (
				<Animated.View style={[styles.top, animatedTopStyles]}>
	...
	renderBottom() {
		...
		let bottomOpacity = pan.y.interpolate({
			inputRange: [SWIPE_THRESHOLD, SWIPE_THRESHOLD / 2],
			outputRange: [0, 1],
			extrapolate: 'clamp'
		});
		let bottomScale = pan.x.interpolate({
			inputRange: [SWIPE_THRESHOLD, 0, -SWIPE_THRESHOLD],
			outputRange: [1, 0, 0],
			extrapolate: 'clamp'
		});
		...
		if (this.props.showBottom) {
			return (
				<Animated.View style={[styles.bottom, animatedBottomStyles]}>
